/** Raw project definitions as tab-separated values: fullCode, requiresClient, description */
...
Business: Outreach	sometimes	Conferences, coffee, donuts, cold calls, research, etc
...
/** Parse the raw project codes into structured data. */
export const parseProjectCodes = () =>
  projectCodes.split("\n").map(line => {
    const [fullCode, requiresClient, description] = line.trim().split("\t")
    const [code, subCode] = fullCode.split(/:\s*/).map(s => s.trim().replaceAll(" ", "-"))
    return { fullCode, code, subCode, requiresClient: requiresClient === "yes", description }
  })
